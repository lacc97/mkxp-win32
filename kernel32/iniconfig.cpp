...
std::optional<std::vector<std::string>> INIConfiguration::Section::getStringListProperty(const std::string& name) const {
  try {
    std::string_view propVal = m_PropertyMap.at(str::to_lower(name)).m_Value;

    std::vector<std::string> list = str::tokenize(propVal, ",");
    std::for_each(list.begin(), list.end(), [](std::string& s) { s = str::view::trim(s); });
...
bool INIConfiguration::save(std::ostream& outStream) const {
  ...
  std::for_each(m_SectionMap.begin(), m_SectionMap.end(), [&](const std::pair<std::string, Section>& selem) {
    ...
    std::for_each(selem.second.m_PropertyMap.begin(), selem.second.m_PropertyMap.end(),
                  [&](const Section::property_map::value_type& pelem) { outStream << pelem.second.m_Name << "=" << pelem.second.m_Value << std::endl; });
...
std::vector<std::string> INIConfiguration::getStringListProperty(const std::string& sName, const std::string& name, const std::vector<std::string>& def) const {
  auto sectionOptional = getSection(sName);
...
void INIConfiguration::setStringListProperty(const std::string& sName, const std::string& name, const std::vector<std::string>& val) {
  if(!getSection(sName))
    addSection(sName);
